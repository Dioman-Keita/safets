 ...
 *
 * Usage:
 *   npx ts-node safets-v06.ts doctor
 *   npx ts-node safets-v06.ts doctor --baseline
 *   npx ts-node safets-v06.ts doctor --fail-on-new
 *   npx ts-node safets-v06.ts fix
 *   npx ts-node safets-v06.ts debt
 *   npx ts-node safets-v06.ts baseline
 *
...
const SKIP_DIRS = new Set([
  ...
  "coverage", ".storybook", ".generated", "generated",
  "tmp", ".cache", "__generated__", ".turbo", ".vercel",
]);
...
function loadProgramRobust(projectRoot: string): ProgramResult {
      ...
      if (!error) {
        ...
        if (fileNames.length > 0) {
          ...
          if (isCheckerUsable(program)) {
            if (errors.length > 0) {
              warnings.push(`tsconfig has ${errors.length} issue(s) — analysis may be partial`);
            }
            return { program, fallback: false, warnings };
          }
  ...
  return { program: null, fallback: true, warnings };
}
...
function getUserSourceFiles(program: ts.Program): ts.SourceFile[] {
  try {
    return program.getRootFileNames()
      ...
      .filter((sf): sf is ts.SourceFile =>
        sf !== undefined && !sf.isDeclarationFile && !sf.fileName.includes("node_modules")
      );
  } catch {
...
function isInsideTryCatch(node: ts.Node): boolean {
  ...
  while (cur) { if (ts.isTryStatement(cur)) return true; cur = cur.parent; }
  return false;
}
...
function isSubChainDuplicate(node: ts.PropertyAccessExpression, checker: ts.TypeChecker): boolean {
  const parent = node.parent;
  if (!ts.isPropertyAccessExpression(parent)) return false;
  try { return isNullable(checker.getTypeAtLocation(parent.expression)); } catch { return false; }
}

function isOptionalAccess(node: ts.PropertyAccessExpression): boolean {
  if (node.questionDotToken !== undefined) return true;
  return ts.isPropertyAccessExpression(node.parent) && node.parent.questionDotToken !== undefined;
}
...
function detectFallbackPatterns(sf: ts.SourceFile): CrashReport[] {
  ...
  function visit(node: ts.Node) {
    ...
    if (
      ...
      results.push({
        file: sf.fileName, line, col,
        expr: node.getText(), rootExpr: "JSON.parse",
        type: "unknown", pattern: "Unprotected JSON.parse",
        confidence: "HIGH", fallback: true,
        crashPath: [
    ...
    if (
      ...
      results.push({
        file: sf.fileName, line, col,
        expr: node.getText(), rootExpr: `process.env.${envVar}`,
        type: "string | undefined", pattern: "Unsafe process.env access",
        confidence: "HIGH", fallback: true,
        crashPath: [
...
function detectUnsafePropertyAccess(sf: ts.SourceFile, checker: ts.TypeChecker): CrashReport[] {
  const results: CrashReport[] = [];
  function visit(node: ts.Node) {
    if (ts.isPropertyAccessExpression(node)) {
      if (isOptionalAccess(node)) { ts.forEachChild(node, visit); return; }
      if (hasNonNullAssertion(node)) { ts.forEachChild(node, visit); return; }
      if (isSubChainDuplicate(node, checker)) { ts.forEachChild(node, visit); return; }
      try {
        ...
        if (isNullable(objType)) {
          ...
          results.push({
            file: sf.fileName, line, col,
            expr: node.getText(), rootExpr: getChainRoot(node.expression).getText(),
            type: checker.typeToString(objType),
            pattern: "Unsafe property access", confidence: "HIGH",
            crashPath: [
      ...
      } catch { /* skip nodes where type resolution fails */ }
    }
    ts.forEachChild(node, visit);
  }
  visit(sf);
...
function detectUnsafeDestructuring(sf: ts.SourceFile, checker: ts.TypeChecker): CrashReport[] {
  const results: CrashReport[] = [];
  function visit(node: ts.Node) {
    if (ts.isVariableDeclaration(node) && node.initializer && ts.isObjectBindingPattern(node.name)) {
      try {
        ...
        if (isNullable(t)) {
          ...
          results.push({
            file: sf.fileName, line, col,
            expr: `const ${node.name.getText()} = ${initText}`, rootExpr: initText,
            type: checker.typeToString(t), pattern: "Unsafe destructuring", confidence: "HIGH",
            crashPath: [`${initText} → ${checker.typeToString(t)}`, `Cannot destructure property of undefined`],
          });
        }
      } catch { /* skip */ }
    }
...
function detectUnsafeArrayAccess(sf: ts.SourceFile, checker: ts.TypeChecker): CrashReport[] {
  const results: CrashReport[] = [];
  function visit(node: ts.Node) {
    if (
      ...
      try {
        ...
        if (isNullable(t)) {
          ...
          results.push({
            file: sf.fileName, line, col,
            expr: node.getText(), rootExpr: arrExpr,
            type: checker.typeToString(t), pattern: "Unsafe array index access", confidence: "HIGH",
            crashPath: [
  ...
  function visit(node: ts.Node) {
    if (
      ...
      results.push({
        file: sf.fileName, line, col,
        expr: node.getText(), rootExpr: "JSON.parse",
        type: "unknown", pattern: "Unprotected JSON.parse", confidence: "HIGH",
        crashPath: [
          "JSON.parse(input) — throws SyntaxError if input is malformed",
          "Unhandled exception → process crash",
        ],
...
function detectUnsafeEnvAccess(sf: ts.SourceFile, checker: ts.TypeChecker): CrashReport[] {
  const results: CrashReport[] = [];
  function visit(node: ts.Node) {
    if (
      ts.isPropertyAccessExpression(node) &&
      ...
      try {
        ...
        if (isNullable(envType)) {
          ...
          results.push({
            ...
            pattern: "Unsafe process.env access", confidence: "HIGH",
            crashPath: [
    ...
    if (
      ...
      results.push({
        file: sf.fileName, line, col,
        expr: node.getText(), rootExpr: `process.env.${envVar}`,
        type: "string | undefined", pattern: "Unsafe process.env access", confidence: "HIGH",
        crashPath: [
...
function detectNonNullAssertionOnNullable(sf: ts.SourceFile, checker: ts.TypeChecker): CrashReport[] {
  const results: CrashReport[] = [];
  function visit(node: ts.Node) {
    if (ts.isNonNullExpression(node)) {
      if (node.expression.getText().startsWith("process.env")) { ts.forEachChild(node, visit); return; }
      try {
        ...
        if (isNullable(innerType)) {
          ...
          results.push({
            file: sf.fileName, line, col,
            expr: node.getText(), rootExpr: node.expression.getText(),
            type: checker.typeToString(innerType),
            pattern: "Non-null assertion on nullable", confidence: "MEDIUM",
            crashPath: [
...
function detectUnsafeAccessAfterAwait(sf: ts.SourceFile, checker: ts.TypeChecker): CrashReport[] {
  const results: CrashReport[] = [];
      ...
      if (ts.isIfStatement(node)) {
        ...
        if (ts.isBinaryExpression(cond)) {
          ...
          if (op === ts.SyntaxKind.EqualsEqualsEqualsToken || op === ts.SyntaxKind.EqualsEqualsToken) {
            ...
            if ((right === "null" || right === "undefined") && ts.isIdentifier(cond.left)) {
        ...
        if (varName) {
          ...
          const isEarlyReturn =
            (ts.isBlock(thenStmt) && thenStmt.statements.length === 1 && ts.isReturnStatement(thenStmt.statements[0])) ||
          ...
          if (!isOptionalAccess(node) && !hasNonNullAssertion(node) && !isSubChainDuplicate(node, checker)) {
            try {
              ...
              if (isNullable(origType)) {
                ...
                results.push({
                  file: sf.fileName, line, col,
                  expr: node.getText(), rootExpr: varName,
                  type: checker.typeToString(origType),
                  pattern: "Unsafe access after await", confidence: "MEDIUM",
                  crashPath: [
...
function detectUnsafePromiseAllDestructuring(sf: ts.SourceFile, checker: ts.TypeChecker): CrashReport[] {
  const results: CrashReport[] = [];
  function visit(node: ts.Node) {
    if (
      ...
      if (
        ...
        try {
          ...
          if (isNullable(initType) || typeStr.includes("undefined")) {
            ...
            results.push({
              file: sf.fileName, line, col,
              expr: node.getText(), rootExpr: "Promise.all",
              type: typeStr, pattern: "Unsafe Promise.all destructuring", confidence: "MEDIUM",
              crashPath: [
...
function detectUnsafeMapAccess(sf: ts.SourceFile, checker: ts.TypeChecker): CrashReport[] {
  const results: CrashReport[] = [];
  function visit(node: ts.Node) {
    if (
      ts.isPropertyAccessExpression(node) &&
      ...
      try {
        ...
        const isMapLike =
          containerStr.includes("Record<") || containerStr.includes("Map<") ||
          containerStr.includes("{ [") || containerStr.includes("Index");
        if (isMapLike) {
          ...
          if (isNullable(elemType)) {
            ...
            results.push({
              file: sf.fileName, line, col,
              expr: node.getText(), rootExpr: mapExpr,
              type: checker.typeToString(elemType), pattern: "Unsafe Map/Record access", confidence: "HIGH",
              crashPath: [
...
function analyze(programResult: ProgramResult): CrashReport[] {
  ...
  const sourceFiles = getUserSourceFiles(programResult.program);
  const all: CrashReport[] = [];
  ...
  for (const sf of sourceFiles) {
    ...
    } catch { /* skip files that cause analysis to fail */ }
  }
  return all;
}
...
function saveBaseline(crashes: CrashReport[], root: string) {
  const b: Baseline = {
    ...
    crashes: crashes.map(cr => ({ file: cr.file, line: cr.line, expr: cr.expr })),
  };
...
function isNew(cr: CrashReport, base: Baseline): boolean {
  return !base.crashes.some(b => b.file === cr.file && b.line === cr.line && b.expr === cr.expr);
}
...
function printDoctor(
  crashes: CrashReport[], root: string,
  failOnNew: boolean, base: Baseline | null,
  pr: ProgramResult
) {
  ...
  if (pr.warnings.length > 0) {
    ...
    if (pr.fallback) console.log(c.yellow("  ⚠ Fallback mode — partial results only\n"));
    else console.log();
  }

  if (crashes.length === 0) {
    console.log(c.green("✓ No potential runtime crashes found.\n"));
    if (pr.fallback) console.log(c.dim("  Note: fallback mode may miss type-dependent crashes.\n"));
    return;
  }
  ...
  if (fallbackCount > 0) console.log(c.dim(`  ${fallbackCount} in fallback mode (lower confidence)`));
  console.log();
  ...
  for (const [file, list] of grouped) {
    ...
    for (const cr of list) {
      const badge   = base ? (isNew(cr, base) ? c.red(" [NEW]   ") : c.dim(" [known] ")) : "";
...
const programResult = loadProgramRobust(root);
const crashes       = analyze(programResult);
const baseline      = loadBaseline(root);

switch (command) {
  case "debt":     printDebt(crashes); break;
  case "fix":      printFix(crashes, root); break;
  case "baseline": saveBaseline(crashes, root); break;
  case "doctor":
  default:
